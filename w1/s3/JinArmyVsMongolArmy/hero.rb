...
class Hero < Warrior
  ...
  def take_turn(enemy_army, own_army)
    ...
    loop do
      ...
      begin
        ...
        if hero_choice < 1 || hero_choice > callable_list.length
          raise StandardError
        end
  ...
  def ask_to_attack(enemy_army)
    ...
    loop do
      ...
      begin
        hero_choice = gets.chomp.to_i
        if hero_choice < 1 || hero_choice > enemy_army.length
          raise StandardError
        end
  ...
  def ask_to_heal(own_army)
    ...
    loop do
      ...
      begin
        hero_choice = gets.chomp.to_i
        if hero_choice < 1 || hero_choice > own_army.length - 1
          raise StandardError
        end
    ...
    ally_to_be_healed = own_army.get(hero_choice - 1, exclude = [self])
